...
Uint16 AD5175_PmtReadWord(AD5175Driver* ad5175, Uint8 addr)
{
    Uint8 addr_read = addr | 0x01;
    Uint16 result = 0;
    Uint8 data[2] = {0};
    ...
    //result += AD5175_ReadByte(ad5175);
    data[0] = AD5175_ReadByte(ad5175);
    AD5175_SendAck(ad5175);  //发送应答ACK
    //result<<=8;
    //result +=  AD5175_ReadByte(ad5175);
    data[1] = AD5175_ReadByte(ad5175);
    AD5175_SendNoAck(ad5175);  //发送NACK

    AD5175_Stop(ad5175);

    result = (data[0]<<8) + data[1];
    TRACE_INFO("\nRead d0 = 0x%02x, d1 = 0x%02x, ret = 0x%04x", data[0], data[1], result);

    return result;
}

Bool AD5175_PmtWriteWord(AD5175Driver* ad5175, Uint8 addr, Uint16 word)
{
    Uint8 addr_wirte = addr;
    Uint8 msb = (Uint8)((word>>8)&0xFF);
    Uint8 lsb = (Uint8)(word&0xFF);
...
//先读高字节，再读低字节
Bool AD5175_PmtReadBytes(AD5175Driver* ad5175, Uint8 addr, Uint8* buff, Uint8 len)
{
    Uint8 addr_read = addr | 0x01;
...
//先发高字节，再发低字节
Bool AD5175_PmtWriteBytes(AD5175Driver* ad5175, Uint8 addr, Uint8* buff, Uint8 len)
{
    Uint8 addr_write = addr;

    AD5175_Start(ad5175);
    AD5175_SendByte(ad5175, addr_write);
    if(!AD5175_WaitAck(ad5175))
    {
        TRACE_ERROR("\n ad5175 write word send addr fail : nack");
        return FALSE;
    ...
    for(Uint8 i = 0; i < len; i++)
    {
        AD5175_SendByte(ad5175, buff[i]);
        if(!AD5175_WaitAck(ad5175))
        {
            TRACE_ERROR("\n pmt write byte:%d value:%X fail : nack", i, buff[i]);
            return FALSE;
        }
    }
    AD5175_Stop(ad5175);
