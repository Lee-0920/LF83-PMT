...
//             TRACE_ERROR("\n Rouse I2C Get Flag Status error");
    ...
    EnvTempCollect_loadTimeoutValve(I2C_TIMEOUT);
    while(FALSE == I2C_CheckEvent(I2C1, I2C_EVENT_MASTER_MODE_SELECT))
    {
...
//            TRACE_ERROR("\n Rouse I2C START error");
    ...
    I2C_Send7bitAddress(I2C1, I2C_SLAVE_ADDRESS, I2C_Direction_Transmitter);// 1 发送从机地址
    while(FALSE == I2C_CheckEvent(I2C1, I2C_EVENT_MASTER_TRANSMITTER_MODE_SELECTED))
    {
        if(TRUE == EnvTempCollect_IsTimeout())
        {
//            TRACE_ERROR("\n Rouse I2C Send7bitAddress error");
    ...
    I2C_SendData(I2C1, TMP75_ADDR_TEMP);    // 2 发送寄存器地址

    EnvTempCollect_loadTimeoutValve(I2C_TIMEOUT);
    while(FALSE == I2C_CheckEvent(I2C1, I2C_EVENT_MASTER_BYTE_TRANSMITTED))
    {
        if(TRUE == EnvTempCollect_IsTimeout())
        {
...
//            TRACE_ERROR("\n Receive I2C START error");
    ...
    EnvTempCollect_loadTimeoutValve(I2C_TIMEOUT);
    while(FALSE == I2C_CheckEvent(I2C1, I2C_EVENT_MASTER_RECEIVER_MODE_SELECTED))
    {
        if(TRUE == EnvTempCollect_IsTimeout())
        {
//            TRACE_ERROR("\n Receive I2C Send7bitAddress error");
    ...
    {
        if(TRUE == I2C_CheckEvent(I2C1, I2C_EVENT_MASTER_BYTE_RECEIVED))
        {
...
//            TRACE_ERROR("\n Init I2C START Init_error");
    ...
    I2C_Send7bitAddress(I2C1, I2C_SLAVE_ADDRESS, I2C_Direction_Transmitter);// 1 发送IIC从地址
...
//            TRACE_ERROR("\n Init I2C Send7bitAddress Init_error");
    ...
    I2C_SendData(I2C1, TMP75_ADDR_CONF);    // 2 发送配置寄存器地址
...
//            TRACE_ERROR("\n Init I2C Send cmd Init_error");
    ...
    I2C_SendData(I2C1, TMP75_MODE); // 3 配置工作模式
