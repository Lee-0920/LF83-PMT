...
void ThermostatManager_Init()
{
    ...
    index = MEAROOM_HEATER;
    Thermostat_SetHeater(&s_thermostats[0], 1, &index);
    index = MEAROOM_COOLER;
    Thermostat_SetRefrigerator(&s_thermostats[0], 1, &index);

	Thermostat_Init(&s_thermostats[1], kDefaultThermostatParam1);
	index = BACTROOM_COOLER;
    Thermostat_SetTemp(&s_thermostats[1], BACTROOM_TEMP);
