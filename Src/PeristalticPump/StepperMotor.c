...
Bool StepperMotor_AbsolutelyMove(Uint8 index, Direction dir,  Uint32 step,  Uint32 speed,  Uint32 acc)
{
    ...
    setparam.acceleration = acc;
    setparam.maxSpeed = speed;
...
Bool StepperMotor_Start(StepperMotor* stepperMotor, Direction dir, Uint32 step, Bool isUseDefaultParam, StepperMotor_OtherMoveHandler otherMoveHandler)
{
    ...
    stepperMotor->otherMoveHandler = otherMoveHandler;
    StepperMotor_MoveStatusSwitch(stepperMotor, MOVE_STATUS_START);

    stepperMotor->remainStep = step * stepperMotor->subdivision;
    ...
    StepperMotorDriver_SetDirection(&stepperMotor->driver, dir);
    StepperMotorTimer_SpeedSetting(stepperMotor, STEPPERMOTOR_INIT_SPEED * stepperMotor->subdivision);

    StepperMotorDriver_Enable(&stepperMotor->driver);
    StepperMotorTimer_Start(stepperMotor);
...
{
    return StepperMotorDriver_ReadDiagnostic(&stepperMotor->driver);
}
