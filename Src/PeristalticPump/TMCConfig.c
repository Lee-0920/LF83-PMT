...
//    return TRUE;
    ...
    if(slaveAddr == 3)
    {
        TMCConfig_CurrentSet(slaveAddr, 0, 2, 4);               //小电流设置
    }
    else if (slaveAddr == 2)
    {
        TMCConfig_CurrentSet(slaveAddr, 5, 31, 4);               //注射器配置
    }
    else
    {
        TMCConfig_CurrentSet(slaveAddr, 5, 23, 4);               //电流设置
    }
...
void TMCMonitor_Handle(TaskHandle_t argument)
{
    while(1)
    {
        switch(s_tmcMonitorStatus)
        {
            case TMC_INIT:
                System_Delay(10);
                TMCConfig_MotorDriverInit(DisplacementMotor_GetStepperMotorX());
                System_Delay(10);
                TMCConfig_MotorDriverInit(DisplacementMotor_GetStepperMotorZ());
                System_Delay(10);
                TMCConfig_MotorDriverInit(SyringeManager_GetStepperMotor());
                System_Delay(10);
                TMCConfig_MotorDriverInit(PeristalticPumpManager_GetStepperMotor(0));
