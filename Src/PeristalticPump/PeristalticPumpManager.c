...
void PeristalticPumpManager_Init(void)
{
    ...
    {
        StepperMotor_SetSubdivision(&g_peristalticPumps[i].stepperMotor, 16);//设置运动参数时需要提前设置细分数
        ...
        StepperMotor_SetNumber(&g_peristalticPumps[i].stepperMotor, 3);
        StepperMotor_Init(&g_peristalticPumps[i].stepperMotor, param);
        StepperMotor_RegisterFinishHandle(&g_peristalticPumps[i].stepperMotor, PeristalticPumpManager_FinishHandler);
...
void PeristalticPumpManager_FinishHandler(void *obj)
{
    ...
    Uint8 number = StepperMotor_GetNumber(stepperMotor) - PUMP_STEP_MOTOR_OFFSET;
    if(TRUE == g_peristalticPumps[number].isSendEvent)
    {
        ...
        Uint8 data[2] = {0};
        data[0] = number + PERISTALTICPUMP_OFFSET;
        data[1] = pumpResult;
...
Uint16 PeristalticPumpManager_Start(Uint8 index, Direction dir, float volume, Bool isUseDefaultParam)
{
    if (index < PERISTALTICPUMPMANAGER_TOTAL_PUMP && volume > 0 && dir < MAX_DIRECTION)
    {
        if (StepperMotor_IDLE == StepperMotor_GetStatus(&g_peristalticPumps[index].stepperMotor))
        {
            ...
            TRACE_INFO(" ml");
            if  (TRUE == StepperMotor_Start(&g_peristalticPumps[index].stepperMotor, dir, step, isUseDefaultParam, StepperMotor_DiagnosticCheck))  ///<驱动诊断
            {
